...
async function shakeSign() {
  let bodyRe = {"floorToken": "df6e1996-0f6f-4c29-92b9-22416fc1f68a", "dataSourceCode": "popUpInfo", "argMap": {}};
  const res = await pg_interact_interface_invoke(bodyRe);
  ...
  if (res.success && res.data) {
    if (res['data']['popUpRemainTimes'] > 0) {
      ...
      console.log(`京东会员第${resCp['data']['floorInfoList']['1']['floorData']['signActInfo']['currSignCursor']}天签到结果；${JSON.stringify(signRes)}`)
      if (signRes.success && signRes['data']) {
        console.log(`京东会员第${resCp['data']['floorInfoList']['1']['floorData']['signActInfo']['currSignCursor']}天签到成功。获得${signRes['data']['rewardVos'] && signRes['data']['rewardVos'][0]['jingBeanVo'] && signRes['data']['rewardVos'][0]['jingBeanVo']['beanNum']}京豆\n`)
      }
    } else {
      console.log(`京东会员第${resCp['data']['floorInfoList']['1']['floorData']['signActInfo']['currSignCursor']}天已经签到了`)
    }
  }
}
function pg_channel_page_data(body) {
  return new Promise(resolve => {
    const options = {
      url: `https://api.m.jd.com/?appid=sharkBean&functionId=pg_channel_page_data&body=${escape(JSON.stringify(body))}`,
      headers: {
        'Cookie': cookie,
        'Host': 'api.m.jd.com',
        'User-Agent': $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.2.2;14.2;%E4%BA%AC%E4%B8%9C/9.2.2 CFNetwork/1206 Darwin/20.1.0"),
