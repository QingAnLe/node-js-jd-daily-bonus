    ...
    if (cookiesArr[i]) {
      ...
      if ((cookiesArr && cookiesArr.length >= 1) && $.canHelp) {
        ...
        // for (let item of $.temp) {
          console.log(`\n${$.UserName} 去参助力 ${$.temp[0].trim()}`);
          const helpRes = await toHelp($.temp[0].trim());
...
function updateShareCodesCDN(url = 'https://TESTcdn.jsdelivr.net/gh/gitupdate/updateTeam@master/shareCodes/jd_cityShareCodes.json') {
  return new Promise(resolve => {
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `http://TESTshare.turinglabs.net/api/v3/carnivalcity/query/20/`, 'timeout': 20000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
