...
!(async () => {
  ...
  await getAuthorShareCode('https://TESTa.nz.lu/jd_zz.json');
  await getAuthorShareCode('https://TESTraw.githubusercontent.com/gitupdate/updateTeam/master/shareCodes/jd_zz.json');
  await getRandomCode();
...
async function distributeBeanActivity() {
  try {
    ...
    if ($.tuan && $.tuan.hasOwnProperty('assistedPinEncrypted') && $.assistStatus !== 3) {
      ...
      $.http.post({
        url: `http://TESTgo.chiang.fun/autocommit`,
...
async function getRandomCode() {
  await $.http.get({url: `http://TESTgo.chiang.fun/read/zuan/${randomCount}`, timeout: 10000}).then(async (resp) => {
    if (resp.statusCode === 200) {
      try {
        let { body } = resp;
        body = JSON.parse(body);
