...
!(async () => {
  ...
  $.authorTuanList = [];
  await requireConfig();
  if (helpAuthor) await getAuthorShareCode('http://TESTqr6pzoy01.hn-bkt.clouddn.com/jd_zz.json');
  if (helpAuthor) await getAuthorShareCode('https://TESTraw.githubusercontent.com/gitupdate/updateTeam/master/shareCodes/jd_zz.json');
  if (!cookiesArr[0]) {
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://TESTcode.chiang.fun/api/v1/jd/jdzz/read/${randomCount}/`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
