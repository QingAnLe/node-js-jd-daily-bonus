...
let inviteCode = [];
if (process.env.JD_CRAZYJOY_SHARECODES) {
  inviteCode.push(process.env.JD_CRAZYJOY_SHARECODES)
};
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://TESTcode.chiang.fun/api/v1/jd/jdcrazyjoy/read/${randomCount}/`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
